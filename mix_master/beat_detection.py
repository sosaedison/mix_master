...
def custom_bpm_detector(y, sr):
    ...
    onset_env_smooth = scipy.signal.savgol_filter(
        onset_env, window_length=51, polyorder=3
    )
    ...
    if lag_max > len(autocorr):
        print(
            f"Adjusting lag range as the original lag_max {lag_max} exceeds autocorrelation length."
        )
    ...
    if lag_min >= len(autocorr) or lag_max > len(autocorr):
        raise ValueError(
            "Lag range exceeds autocorrelation length after adjustment. Audio may not have enough rhythmic content."
        )
    ...
    if len(autocorr_slice) == 0:
        raise ValueError(
            "Autocorrelation slice is empty. Audio may lack rhythmic content or be too short."
        )

    # Check if we have valid peaks
    peaks, properties = scipy.signal.find_peaks(
        autocorr_slice, height=np.max(autocorr_slice) * 0.8
    )
    ...
    tempo_librosa = librosa.feature.rhythm.tempo(
        y=y, sr=sr, start_bpm=125, aggregate=None
    )
